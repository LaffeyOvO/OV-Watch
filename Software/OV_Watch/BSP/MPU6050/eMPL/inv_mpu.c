...
u8 mpu_dmp_init(void)
{
    ...
    if (mpu_init() == 0) // ��ʼ��MPU6050
    {
        ...
        if (res)
            return -1;
        // res=mpu_set_sensors(INV_XYZ_ACCEL);//��������Ҫ�Ĵ�����
        // if(res)return 1;
        res = mpu_configure_fifo(INV_XYZ_ACCEL); // ����FIFO
        if (res)
            return 2;
        ...
        // if(res)return 5;
        res = dmp_enable_feature(DMP_FEATURE_TAP | DMP_FEATURE_SEND_RAW_ACCEL | DMP_FEATURE_PEDOMETER); // ����dmp����
        if (res)
            return 6;
